 ...
 */
export function getStationsToFetch(
  ...
  for (let i = 0; i < toFetch.length; i += batchSize) {
    ...
    try {
      const res = await fetch('/api/matrix', {
        ...
        body: JSON.stringify({
          locations: [
            [internalUserCoords.lon, internalUserCoords.lat],
            ...batch.map((r) => [parseFloat(r.lng as string), parseFloat(r.lat as string)]),
          ],
          sources: [0],
      ...
      batch.forEach((row, idx) => {
        if (result.durations && result.durations[0][idx] !== null) {
          ...
          console.warn(
            `ORS returned null duration for station: ${row.Station} at ${row.Address} (Coords: ${row.lat}, ${row.lng})`
          );
        }
      });
      ...
      uiCallbacks.alert(
        `Failed to calculate driving times: ${e.message}. Check console for details.`
      );
      uiCallbacks.setStatus('', false);
      uiCallbacks.setButtonState('Calculate Driving Times', false);
