...
describe("Astro Master Route Manifest", () => {
  it("verifies the integrity of the entire generated routing table", () => {
    ...
    manifest.forEach(item => {
      ...
      expect(seenRoutes.has(item.fullPath), `Duplicate Route: ${item.fullPath} (from ${item.id})`).toBe(false);
      ...
      expect(fs.existsSync(publicConflictPath), `Route Collision: Route ${item.fullPath} shadows a physical folder in public/`).toBe(false);
    });
