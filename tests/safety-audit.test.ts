...
const blogFiles = getFilesRecursively(
  BLOG_PATH,
  (file) => file.endsWith('.md') || file.endsWith('.mdx')
);
const srcFiles = getFilesRecursively(SRC_PATH, (file) =>
  /\.(astro|md|mdx|ts|tsx|js|jsx)$/.test(file)
);
...
describe('Pagefind Safety Audit', () => {
  it.each(srcFiles)('ensuring no data-pagefind-body in %s', (file) => {
    ...
    expect(
      content,
      `Found "${forbidden}" in ${file}. Pagefind should index the whole page by default.`
