...
async function run() {
  ...
  try {
    const res = await Promise.all([
      ...
      // 7 Day (Requesting 10 to ensure layout flexibility)
      fetch(`${base}?method=user.gettopartists&period=7day&limit=10&${params}`),
      fetch(`${base}?method=user.gettopalbums&period=7day&limit=10&${params}`),
      ...
      fetch(`${base}?method=user.gettopalbums&period=1month&limit=10&${params}`),
      ...
      fetch(`${base}?method=user.gettopalbums&period=12month&limit=10&${params}`),
      ...
      fetch(`${base}?method=user.gettopalbums&period=overall&limit=10&${params}`),
